...
class CleanSales:
    ...
    def clean_data(self) -> pd.DataFrame:
        ...
        df.fillna(
            {
                "customer_age": (
                    df["customer_age"].median() if "customer_age" in df.columns else 0
                ),
                "gender": "Unknown",
                "product": "Unknown",
                "quantity": 0,
        ...
        df["customer_age"] = pd.to_numeric(df["customer_age"], errors="coerce").astype(
            int
        )
        ...
        df["spend_category"] = np.where(df["sales_amount"] > median_spent, "A", "B")
